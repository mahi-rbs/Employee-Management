  ...
  <div class="box">
    <div class="box-header">
      <div class="row">
        <div class="col-sm-4">
          ...
          <form action="{{url('system-management/report/bydesignation')}}" method="post">
            {{csrf_field()}}
            <div class="form-group">

              <select class="form-control" id="serchByDesignation" name="serchByDesignation">
                <option value="-1">--search by designation--</option>
                @foreach($designations as $designation)
                <option value="{{$designation->id}}" {{ ($designation->id == $designationId) ? 'selected' : '' }}>{{$designation->name}}</option>
                @endforeach
              </select>
    ...
    <div class="box-body">
      ...
      <form method="POST" action="{{ route('report.search') }}">
       {{ csrf_field() }}
       @component('layouts.search', ['title' => 'Search By Join Date'])
...
<script type="text/javascript">
  $("#serchByDesignation").change(function() {
     this.form.submit();
});
</script>
